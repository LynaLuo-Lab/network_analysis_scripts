...
#TO DO: refactor verbose printing statements elsewhere to use this function
#       so that users can use cat (rstudio) or message (jupyter) for
#       message printing.
verbMsg <- function(verbStr="",useMessage=FALSE,newLine=TRUE){
   if(useMessage){
       message(verbStr,appendLF=newLine)
   } else {
       cat(verbStr)
       if(newLine){cat("\n")}
   }
...
do_windowed_betweenness_analysis <- function(windowed_corr_data,resData,
                                             ...
                                             verbose=FALSE,verboseLevel=0,useMessage=FALSE) {
  ...
  for(system in systems){
    if(verbose){
        verbStr="------------------------------"
        verbMsg(verbStr,useMessage)
        verbStr=paste("Working on ",system," system")
        verbMsg(verbStr,useMessage)
    }
    ...
    for(window in windows){
        if(verbose){
            verbStr=paste("-Working on window",window)
            verbMsg(verbStr,useMessage)
        }
      ...
      if(verbose && (verboseLevel > 0)){
          verbStr="--source sets:"
          verbMsg(verbStr,useMessage,newLine=FALSE)
          verbStr=paste(sources,collapse=", ")
          verbMsg(verbStr,useMessage)
          verbStr="--target sets:"
          verbMsg(verbStr,useMessage,newLine=FALSE)
          verbStr=paste(targets,collapse=", ")
          verbMsg(verbStr,useMessage)
      }
      ...
      for(source in sources) {
        ...
        for(target in targets) {
          ...
          if(verbose) {
              verbStr=paste("--Working on",
                            paste(source,target,sep="_"),
                            "network")
              verbMsg(verbStr,useMessage)
              if(verboseLevel > 1){
                  verbStr="---Source nodes:"
                  verbMsg(verbStr,useMessage,newLine=FALSE)
                  verbStr=paste(srcnodes,collapse=", ")
                  verbMsg(verbStr,useMessage)
                  verbStr="---Target nodes:"
                  verbMsg(verbStr,useMessage,newLine=FALSE)
                  VerbStr=paste(trgnodes,collapse=", ")
                  verbMsg(verbStr,useMessage)
              }
          }
  ...
  for(system in systems) {
    ...
    for(source in sourceSet) {
      for(target in targetSet) {
        networkName=paste(source,target,sep="_")
		if(verbose & verboseLevel>0){
            verbStr=paste("-working on",networkName,"network")
            verbMsg(verbStr,useMessage)
        }
		for(window in windows) {
        	if(verbose & verboseLevel>0){
        	...
        	for(weightType in weightTypes){
        	  ...
        	  weightFilePath=paste(weightdir,weightFileName,sep="/")
        	  if(!file.exists(weightFilePath)){
        	    verbStr=paste("ERROR: the weightFile",
        	                weightFilePath,"does not exist!")
                verbMsg(verbStr,useMessage)
        	    next
        	  }
        	  ...
        	  for(sourceNode in sourceNodes) {
        	    for(targetNode in targetNodes) {
        	      ...
        	      if(verbose & verboseLevel>1){
                      verbStr=paste("---running subopt program for the",
                                    nodePairName,"source-target pair")
                      verbMsg(verbStr,useMessage)
                  }
        	      outFileName=paste(paste(system,networkName,sep="_"),
        	      ...
        	      outFilePath=paste(outdir,outFileName,sep="/")
        	      if(verbose & verboseLevel>2) {
        	        verbStr=paste("----weightFilePath:",weightFilePath)
                    verbMsg(verbStr,useMessage)
        	        verbStr=paste("----outFilePath:",outFilePath)
                    verbMsg(verbStr,useMessage)
        	      }
        	      system2(suboptexe,
        	              args=c(weightFilePath,outFilePath,
